...
class Token:
    ...
    def __str__(self):
        if (
            ...
            or self.type == TokenType.LNUM
            or self.type == TokenType.LSTRING
        ):
...
class ExprParser:
    ...
    def lex_alpha(self, i):
        ...
        while i < len(self.string) and (
            self.string[i].isalnum() or self.string[i] == "_"
        ):
            i = i + 1
    ...
    def lex_quoted_token(self, i):
        ...
        while i < len(self.string):
            ...
            if (
                c == quote_char
                and i + 1 < len(self.string)
                and self.string[i + 1] != quote_char
            ):
    ...
    def assert_cur_token(self, type):
        ...
        if self.tokens[self.pos].type != type:
            raise ValueError(
                ...
                "type {2}".format(type, self.pos, self.tokens[self.pos])
            )
    ...
    def document_path(self):
        ...
        if items > 0 and docpath[items - 1].type == DocumentPathItem.DOUBLE_ASTERISK:
            raise ValueError("JSON path may not end in '**' at {0}" "".format(self.pos))
        return docpath
    ...
    def column_identifier(self):
        ...
        while self.cur_token_type_is(TokenType.DOT):
            ...
            parts.append(self.consume_token(TokenType.IDENT))
        if len(parts) > 3:
            raise ValueError("Too many parts to identifier at {0}" "".format(self.pos))
        parts.reverse()
    ...
    def mul_div_expr(self):
        return self.parse_left_assoc_binary_op_expr(
            set([TokenType.MUL, TokenType.DIV, TokenType.MOD]), self.atomic_expr
        )
    ...
    def bit_expr(self):
        return self.parse_left_assoc_binary_op_expr(
            set([TokenType.BITAND, TokenType.BITOR, TokenType.BITXOR]), self.shift_expr
        )
    ...
    def ilri_expr(self):
        ...
        if self.pos < len(self.tokens):
            ...
            if self.cur_token_type_is(TokenType.IS):
                ...
                if is_not:
                    raise ValueError(
                        "Unknown token after NOT as pos {0}" "".format(self.pos)
                    )
...
def x_test():
    ...
    parseAndPrintExpr("now () + @b + c > 2")
    parseAndPrintExpr(
        "now () - interval +2 day > some_other_time() or " "something_else IS NOT NULL"
    )
