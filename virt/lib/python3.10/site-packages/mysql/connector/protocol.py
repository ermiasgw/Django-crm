...
class MySQLProtocol(object):
    ...
    def _auth_response(
        ...
        try:
            ...
            raise errors.ProgrammingError("Failed authentication: {0}".format(str(exc)))
    ...
    def parse_ok(self, packet):
        ...
        try:
            ...
            (ok_packet["status_flag"], ok_packet["warning_count"]) = struct_unpack(
                "<HH", packet[0:4]
            )
            packet = packet[4:]
            if packet:
    ...
    def _parse_binary_timestamp(self, packet, field):
        ...
        if length == 4:
            value = datetime.date(
                year=struct_unpack("H", packet[1:3])[0], month=packet[3], day=packet[4]
            )
        elif length >= 7:
            mcs = 0
            if length == 11:
    ...
    def _parse_binary_time(self, packet, field):
        ...
        tmp = datetime.timedelta(
            days=days, seconds=data[7], microseconds=mcs, minutes=data[6], hours=data[5]
        )
    ...
    def _parse_binary_values(self, fields, packet):
        ...
        for pos, field in enumerate(fields):
            if null_bitmap[int((pos + 2) / 8)] & (1 << (pos + 2) % 8):
                ...
                values.append(value)
            elif field[1] in (FieldType.DATETIME, FieldType.DATE, FieldType.TIMESTAMP):
        ...
        if parameters and data:
            ...
            for pos, _ in enumerate(parameters):
                ...
                if value is None:
                    ...
                    types.append(
                        utils.int1store(FieldType.NULL) + utils.int1store(flags)
                    )
                    continue
                    ...
                    values.append(utils.lc_int(len(value)) + value)
                    field_type = FieldType.BLOB
                elif PY2 and isinstance(value, unicode):  # pylint: disable=E0602
                    value = value.encode(charset)
                    values.append(utils.lc_int(len(value)) + value)
                    field_type = FieldType.VARCHAR
