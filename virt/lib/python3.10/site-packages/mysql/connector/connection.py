...
class MySQLConnection(MySQLConnectionAbstract):
    ...
    def _auth_switch_request(self, username=None, password=None):
        ...
        if packet[4] == 254 and len(packet) == 5:
            ...
            # AuthSwitchRequest
            (new_auth_plugin, auth_data) = self._protocol.parse_auth_switch_request(
                packet
            )
    ...
    def get_rows(self, count=None, binary=False, columns=None):
        ...
        if rows[-1] is not None:
            ...
            self._handle_server_status(
                ek["status_flag"] if "status_flag" in ek else ek["server_status"]
            )
            self.unread_result = False
    ...
    def reset_session(self, user_variables=None, session_variables=None):
        ...
        try:
            ...
            self.cmd_change_user(
                self._user, self._password, self._database, self._charset_id
            )
    ...
    def cmd_stmt_execute(self, statement_id, data=(), parameters=(), flags=0):
        ...
        execute_packet = self._protocol.make_stmt_execute(
            statement_id, data, tuple(parameters), flags, long_data_used, self.charset
        )
    ...
    def cmd_stmt_close(self, statement_id):
        ...
        """
        self._send_cmd(
            ServerCmd.STMT_CLOSE, int4store(statement_id), expect_response=False
        )
    ...
    def cmd_stmt_send_long_data(self, statement_id, param_id, data):
        ...
        try:
            ...
            while buf:
                ...
                self._send_cmd(
                    ServerCmd.STMT_SEND_LONG_DATA, packet=packet, expect_response=False
                )
                total_sent += len(buf)
                buf = data.read(chunk_size)
        except AttributeError:
