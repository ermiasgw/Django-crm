...
MYSQL_FABRIC_PORT = {"xmlrpc": 32274, "mysql": 32275}
...
_CNX_PROPERTIES = {
    ...
    "key": (
        tuple([int, str, datetime.datetime, datetime.date] + list(UNICODE_TYPES)),
        "Sharding key",
        None,
    ),
    ...
    "attempts": ((int,), "Attempts for getting connection", _GETCNX_ATTEMPT_MAX),
    "attempt_delay": (
        (int,),
...
class MySQLRPCProtocol(object):
    ...
    def __init__(self, fabric, host, port, connect_attempts, connect_delay):
        ...
        self.handler = FabricMySQLConnection(
...
class XMLRPCProtocol(object):
    ...
    def __init__(self, fabric, host, port, connect_attempts, connect_delay):
        self.handler = FabricXMLRPCConnection(
            fabric, host, port, connect_attempts, connect_delay
        )
        self.handler.connect()
    ...
    def execute(self, group, command, *args, **kwargs):
        ...
        try:
            ...
            raise ValueError(
                "{group}.{command} not available ({err})".format(
        ...
        if format_version != FabricResponse.SUPPORTED_VERSION:
            raise InterfaceError(
                ...
                "from MySQL Fabric with version {gversion}.".format(
                    sversion=FabricResponse.SUPPORTED_VERSION, gversion=format_version
                )
            )
...
class FabricSet(FabricResponse):
    ...
    def __init__(self, data):
        ...
        assert (
            all(len(self.__names) == len(row) for row in self.__rows)
            or len(self.__rows) == 0
        )
        self.__result = collections.namedtuple("ResultSet", self.__names)
...
class FabricTransport(Transport):
    ...
    def request(self, host, handler, request_body, verbose=0):
        """Send XMLRPC request"""
        uri = "{scheme}://{host}{handler}".format(
            scheme=self._scheme, host=host, handler=handler
        )

        if self._passmgr:
            self._passmgr.add_password(None, uri, self._username, self._password)
        if self.verbose:
...
class Fabric(object):
    ...
    def __init__(
        ...
        if protocol == "xmlrpc":
            ...
            raise InterfaceError(
                "Protocol not supported by MySQL Fabric," " was '{}'".format(protocol)
            )
    ...
    def seed(self, host=None, port=None):
        ...
        fabric_uuid, fabric_version, ttl, fabrics = self.get_fabric_servers(fabinst)
    ...
    def report_failure(self, server_uuid, errno):
        ...
        if errno in REPORT_ERRORS or errno in REPORT_ERRORS_EXTRA:
            _LOGGER.debug("Reporting error %d of server %s", errno, server_uuid)
            inst = self.get_instance()
            try:
                data = inst.execute(
                    "threat", "report_failure", server_uuid, current_host, errno
                )
                FabricResponse(data)
            except (Fault, socket.error) as exc:
                _LOGGER.debug("Failed reporting server to Fabric (%s)", str(exc))
    ...
    def get_fabric_servers(self, fabric_cnx=None):
        ...
        try:
            fset = inst.execute("dump", "fabric_nodes", "protocol." + self._protocol)

            for row in fset.rows():
                result.append({"host": row.host, "port": row.port})
        except (Fault, socket.error) as exc:
            msg = err_msg.format(
                err=str(exc), host=inst.handler.host, port=inst.handler.port
            )
            raise InterfaceError(msg)
        except (TypeError, AttributeError) as exc:
            msg = err_msg.format(
                err="No Fabric server available ({0})".format(exc),
                host=inst.handler.host,
    ...
    def get_group_servers(self, group, use_cache=True):
        ...
        try:
            ...
            msg = (
                "Looking up MySQL servers failed for group " "{group}: {error}"
            ).format(error=str(exc), group=group)
            raise InterfaceError(msg)
    ...
    def get_sharding_information(self, tables=None, database=None):
        ...
        try:
            ...
            fset = inst.execute(
                "dump", "sharding_information", self._version_token, ",".join(patterns)
            )
        except (Fault, socket.error) as exc:
    ...
    def set_property(self, **properties):
        ...
        for name, value in properties.items():
            if name not in _CNX_PROPERTIES:
                raise ValueError("Invalid property connection {0}".format(name))
            ...
            if name == "group" and value and (props["key"] or props["tables"]):
                raise ValueError(
                    "'group' property can not be set when 'key' or " "'tables' are set"
                )
            ...
            elif name == "mode" and value not in (MODE_READWRITE, MODE_READONLY):
    ...
    def _configure_fabric(self, config):
        ...
        if isinstance(config, Fabric):
            ...
            try:
                ...
                raise InterfaceError("{0} protocol is not available".format(protocol))
        ...
        while counter != attempts:
            ...
            try:
                ...
                if props["tables"]:
                    ...
                    raise ValueError("Missing group or key and tables properties")
            except InterfaceError as exc:
                _LOGGER.debug(
                    "Trying to get MySQL server (attempt {0}; {1})".format(counter, exc)
                )
                if counter == attempts:
                    raise InterfaceError("Error getting connection: {0}".format(exc))
                if attempt_delay > 0:
    ...
    def handle_mysql_error(self, exc):
        ...
        if exc.errno in RESET_CACHE_ON_ERROR:
            ...
            raise MySQLFabricError(
                "Temporary error ({error}); " "retry transaction".format(error=str(exc))
            )
