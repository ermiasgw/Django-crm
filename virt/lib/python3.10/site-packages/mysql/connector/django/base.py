...
class CursorWrapper(object):
    ...
    def _execute_wrapper(self, method, query, args):
        ...
        try:
            ...
            six.reraise(
                utils.IntegrityError, utils.IntegrityError(err.msg), sys.exc_info()[2]
            )
            ...
            if err.args[0] in self.codes_for_integrityerror:
                six.reraise(
                    ...
                    utils.IntegrityError(err.msg),
                    sys.exc_info()[2],
                )
            else:
                six.reraise(
                    utils.DatabaseError, utils.DatabaseError(err.msg), sys.exc_info()[2]
                )
...
class DatabaseWrapper(BaseDatabaseWrapper):
    ...
    def data_types(self):
        if self.features.supports_microsecond_precision:
            return dict(
                self._data_types, DateTimeField="datetime(6)", TimeField="time(6)"
            )
        else:
    ...
    pattern_esc = (
        r"REPLACE(REPLACE(REPLACE({}, '\\', '\\\\')," r" '%%', '\%%'), '_', '\_')"
    )
    pattern_ops = {
        ...
        for table_name in table_names:
            ...
            for (
                ...
                for bad_row in cursor.fetchall():
                    msg = (
                        "The row in table '{0}' with primary key '{1}' has "
