                ...
                for option, value in config_from_file[group].items():
                    try:
                        ...
                        if option in CNX_FABRIC_ARGS:
                            if (
                                option not in fabric_options
                                or fabric_options[option][1] <= value[1]
                            ):
                                fabric_options[option] = value
                            continue

                        if option not in CNX_POOL_ARGS and option not in [
                            "fabric",
                            "failover",
                        ]:
...
class MySQLOptionsParser(SafeConfigParser):  # pylint: disable=R0901
    ...
    def _parse_options(self, files):
        ...
        if not_read_files:
            raise ValueError(
                "File(s) {0} could not be read.".format(", ".join(not_read_files))
            )
