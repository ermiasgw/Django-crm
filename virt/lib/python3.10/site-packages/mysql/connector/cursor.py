...
RE_SQL_INSERT_STMT = re.compile(
    r"({0}|\s)*INSERT({0}|\s)*INTO.+VALUES.*".format(SQL_COMMENT), re.I | re.M | re.S
)
...
def _bytestr_format_dict(bytestr, value_dict):
    ...
    def replace(matchobj):
        ...
        if groups["conversion_type"] == b"s":
            key = (
                groups["mapping_key"].encode("utf-8") if PY2 else groups["mapping_key"]
            )
            value = value_dict[key]
        if value is None:
            raise ValueError(
                "Unsupported conversion_type: {0}" "".format(groups["conversion_type"])
            )
        return value.decode("utf-8") if PY2 else value

    return RE_PY_MAPPING_PARAM.sub(replace, bytestr.decode("utf-8") if PY2 else bytestr)
...
class MySQLCursor(CursorBase):
    ...
    def _batch_insert(self, operation, seq_params):
        ...
        tmp = re.sub(
            RE_SQL_ON_DUPLICATE, "", re.sub(RE_SQL_COMMENT, remove_comments, operation)
        )
    ...
    def fetchall(self):
        ...
        if hasattr(self._connection, "converter"):
            for row in self._rows[self._next_row :]:
                res.append(
                    self._connection.converter.row_to_python(row, self.description)
                )
        else:
